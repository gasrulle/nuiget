...
- **Multi-project install from sidebar**: When "All Projects" is selected, install actions ("+" icon and context menu "Install Latest" / "Install Version...") use `showQuickPick({ canPickMany: true })` for multi-select. `_pickProjectsForInstall(packageId)` derives the project list from `queryAllProjectsInstalled()` (reusing the installed-data fetch to mark already-installed projects), falling back to `findProjects()` if empty. Single selection → `executeSingleOperation` (existing path). Multiple selections → `executeBulkInstall` (same shared function from `NuGetOperations.ts` that the main panel uses). Non-install actions (Uninstall, Change Version) remain single-select. SidebarApp.tsx handles `bulkInstallResult` with optimistic state updates.
